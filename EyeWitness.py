...
class EyeWitness(object):
    ...
    def worker_thread(self, cli_parsed, targets, lock, counter, user_agent=None):
    ...
    def single_vnc_rdp(self, cli_parsed, engine):
        url = cli_parsed.single
        if engine == 'vnc':
    ...
    def multi_mode(self, cli_parsed):
        ...
        targets = m.Queue()
        lock = m.Lock()
        self.multi_counter = m.Value('i', 0)
        display = None

        def exitsig(self, *args):
            dbm.close()
        ...
        if any((cli_parsed.web, cli_parsed.headless)):
            ...
            self.multi_total = dbm.get_incomplete_http(targets)
            if self.multi_total > 0:
                if cli_parsed.resume:
                    print 'Resuming Web Scan ({0} Hosts Remaining)'.format(str(self.multi_total))
                else:
                    print 'Starting Web Requests ({0} Hosts)'.format(str(self.multi_total))

            if self.multi_total < cli_parsed.threads:
                num_threads = self.multi_total
            else:
                num_threads = cli_parsed.threads
            for i in xrange(num_threads):
                targets.put(None)
            try:
                workers = [Process(target=self.worker_thread, args=(
                    cli_parsed, targets, lock, (self.multi_counter, self.multi_total))) for i in xrange(num_threads)]
                for w in workers:
                    w.start()
                for w in workers:
                    w.join()
            ...
            if cli_parsed.cycle is not None:
                ...
                for browser, ua in ua_dict.iteritems():
                    targets = m.Queue()
                    self.multi_counter.value = 0
                    self.multi_total = dbm.get_incomplete_ua(targets, browser)
                    if self.multi_total > 0:
                        print("[*] Starting requests for User Agent {0}"
                            " ({1} Hosts)").format(browser, str(self.multi_total))
                    if self.multi_total < cli_parsed.threads:
                        num_threads = self.multi_total
                    else:
                        num_threads = cli_parsed.threads
                    for i in xrange(num_threads):
                        targets.put(None)
                    workers = [Process(target=self.worker_thread,
                                    args=(cli_parsed, targets, lock,
                                            (self.multi_counter, self.multi_total),
                                            (browser, ua)))
                           for i in xrange(num_threads)]
                    for w in workers:
                        w.start()
        ...
        if any((cli_parsed.vnc, cli_parsed.rdp)):
            ...
            self.multi_total, targets = dbm.get_incomplete_vnc_rdp()
            if self.multi_total > 0:
                print ''
                print 'Starting VNC/RDP Requests ({0} Hosts)'.format(str(self.multi_total))
    ...
    def multi_callback(self, x):
        self.multi_counter += 1

        if self.multi_counter % 15 == 0:
            print '\x1b[32m[*] Completed {0} out of {1} hosts\x1b[0m'.format(self.multi_counter, self.multi_total)
